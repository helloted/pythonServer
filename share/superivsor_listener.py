    ...
    def runforever(self):
        ...
        while 1:
            ...
            if not headers['eventname'] == 'PROCESS_STATE_EXITED':
                ...
                self.stdout.write('here is not PROCESS_STATE_EXITED')
                self.stdout.flush()
                continue
            else:
                self.stdout.write('here is ****')
            ...
            if int(pheaders['expected']):
                childutils.listener.ok(self.stdout)
                self.stdout.write('here is excpected if')
                self.stdout.flush()
                continue
            else:
                self.stdout.write('here is ========')
...
def main():
    ...
    write_stdout('from now on restart this process monitor \n')
